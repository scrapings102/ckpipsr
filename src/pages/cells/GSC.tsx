...
interface GSCMember {
  ...
  contact: string;
  email: string;
  category: "Leadership" | "Faculty" | "Technical Staff" | "Student";
}

const gscMembers: GSCMember[] = [
  {
    role: "Chairman",
...
export default function GSC() {
  ...
  const filteredMembers = gscMembers.filter((m) => {
    const matchesSearch =
  ...
  const categories = ["All", "Leadership", "Faculty", "Technical Staff", "Student"];

  return (
    <SubPageLayout
      title="Gender Sensitization Cell (GSC)"
      subtitle="Promoting Gender Equality, Inclusivity & Zero Tolerance for Harassment"
      category="cells"
      activeItemLabel="GSC"
    >
